        ...
        for(p in 1:length(experParams$paraSearchMethod)){
          ...
          accuracyFile <- file.path(outputFolder,
                                    paste(methodNames[m], "_",
                                          ...
                                          experParams$currentSearchMethod,".csv", sep = ""))#得到文件存储地址
          write.csv(accuracyData, file = accuracyFile, row.names = FALSE)#accuracyData中存储着结果
...
AnalysePredError <- function(sysDataInfo, methodInfo) {
  ...
  # Monitor()//会产生"FUNCTION: AnalysePredError"的信息，并不是报错
  ...
  resultFormat <- c(resultFormat, "explained_variance_score")
  resultFormat <- c(resultFormat, "mean_absolute_error")
  resultFormat <- c(resultFormat, "root_mean_squared_error")
  resultFormat <- c(resultFormat, "r2_score")
  
  
  resultData <- rbind(resultData, 1:length(resultFormat))
  colnames(resultData) <- resultFormat

  resultData <- tibble::as_tibble(resultData)
  ...
  for (sampleSizeID in 1:experParams$sampleSizes) {
      ...
      for (sampleID in 1:experParams$sampleRep) {
        ...
        resultRow<-c(resultRow,modelParamsResult$explained_variance_score)
        resultRow<-c(resultRow,modelParamsResult$mean_absolute_error)
        resultRow<-c(resultRow,modelParamsResult$root_mean_squared_error)
        resultRow<-c(resultRow,modelParamsResult$r2_score)
...
GetPredError <- function(methodInfo, sysDataInfo, sampleSizeID, sampleID) {
  ...
  explained_variance_score= 1-var(testData-predicted)[1,1]/var(testData)[1,1]
  mean_absolute_error<- colMeans(abs(testData - predicted))
  root_mean_squared_error<- sqrt(colMeans((testData-predicted)^2))
  r2_score<- 1-(colMeans((testData-predicted)^2)/var(testData))
  ...
  modelParamsResult$explained_variance_score<-explained_variance_score
  modelParamsResult$mean_absolute_error<-mean_absolute_error
  modelParamsResult$root_mean_squared_error<-root_mean_squared_error
  modelParamsResult$r2_score<-r2_score
