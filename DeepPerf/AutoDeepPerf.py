...
def system_samplesize(sys_name):
    ...
    elif(sys_name=='Apache_PCA'):
        N_train_all=np.multiply(9, [1, 2, 3, 4, 5]) # This is for Apache_PCA
    elif (sys_name == 'BDBJ'):
        N_train_all = np.multiply(26, [1, 2, 3, 4, 5])  # This is for BDBJ(源代码26，现18)
    elif(sys_name=='BDBJ_PCA'):
        N_train_all = np.multiply(9, [1, 2, 3, 4, 5])  # This is for BDBJ_PCA
    elif (sys_name == 'BDBC'):
        N_train_all = np.multiply(18, [1, 2, 3, 4, 5])  # This is for BDBC(原代码18，现17)
    elif(sys_name=='BDBC_PCA'):
        N_train_all = np.multiply(15, [1, 2, 3, 4, 5])  # This is for BDBC_PCA
    elif (sys_name == 'LLVM'):
        N_train_all = np.multiply(11, [1, 2, 3, 4, 5])  # This is for LLVM
    elif(sys_name=='LLVM_PCA'):
        N_train_all = np.multiply(11, [1, 2, 3, 4, 5])  # This is for LLVM_PCA
    elif (sys_name == 'SQL'):
        N_train_all = np.multiply(39, [1, 2, 3, 4, 5])  # This is for SQL（原39，现40）
    elif(sys_name=='SQL_PCA'):
        N_train_all = np.multiply(25, [1, 2, 3, 4, 5])  # This is for SQL_PCA
    elif (sys_name == 'x264'):
        N_train_all = np.multiply(16, [1, 2, 3, 4, 5])  # This is for X264（原16，现14）
    elif(sys_name=='x264_PCA'):
        N_train_all = np.multiply(12, [1, 2, 3, 4, 5])  # This is for X264
    elif (sys_name == 'Dune'):
        N_train_all = np.multiply(12, [1, 6, 11, 16, 21, 26])  # This is for Dune(改为n到5n了)
    elif(sys_name=='Dune_PCA'):
        N_train_all = np.multiply(8, [1, 6, 11, 16, 21, 26])  # This is for Dune_PCA(PCA改良前是6)
    elif (sys_name == 'hipacc'):
        N_train_all = np.multiply(34, [1, 6, 11, 16, 21, 26, 31])  # This is for hipacc
    elif(sys_name == 'hipacc_PCA'):
        N_train_all = np.multiply(1, [1, 6, 11, 16, 21, 26, 31])  # This is for hipacc_PCA
    elif (sys_name == 'hsmgp'):
        N_train_all = np.multiply(15, [1, 6, 11, 16, 21, 26, 31]) #np.asarray([77, 173, 384, 480])  # This is for hsmgp
    elif (sys_name == 'hsmgp_PCA'):
        N_train_all = np.multiply(11, [1, 6, 11, 16, 21, 26, 31])# This is for hsmgp_PCA
...
if __name__ == '__main__':
    ...
    dir_data = 'Data_PCA/' + sys_name + '_AllNumeric.csv'
    ...
    for idx in range(len(sample_size_all)):
        ...
        for m in range(1, n_exp+1):
            ...
            explained_variance_score.append(metrics.explained_variance_score(Y_test.ravel(), Y_pred_test.ravel()))
            mean_absolute_error.append(metrics.mean_absolute_error(Y_test.ravel(), Y_pred_test.ravel()))
            root_mean_squared_error.append(metrics.mean_squared_error(Y_test.ravel(), Y_pred_test.ravel())**0.5)
            r2_score.append(metrics.r2_score(Y_test.ravel(), Y_pred_test.ravel()))
        ...
        result["explained_variance_score"] = explained_variance_score
        ...
        for i in range(len(result_sys)):
            ...
            result_exp = [temp['N_train'], np.mean(temp['rel_error_mean']),np.mean(temp['explained_variance_score']),np.mean(temp['mean_absolute_error']),
                          np.mean(temp['root_mean_squared_error']),np.mean(temp['r2_score']),
                          ci_temp]
        ...
        np.savetxt(filename, result_arr, fmt="%f", delimiter=",",
                   header="Sample size, Mean, explained_variance_score, mean_absolute_error, root_mean_squared_error, r2_score, Margin")
