...
from util import *
from data_divide import makedata
...
for x in range(6):
    ...
    for k in range(30):
        ...
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr_model,
                               weight_decay=weight_decay)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=10)  # 学习率衰减
        ...
        record = pd.DataFrame([], columns=['epoch', 'train_mse', 'train_rank_spearman',
                                           'valid_mre', 'valid_mse', 'valid_rank_spearman', 'test_mre', 'test_mse',
                                           'test_rank_spearman', 'explained_variance_score', 'mean_absolute_error',
        ...
        for epoch in range(max_epoch):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                real_labels = sample_real_labels(batch_x.size(0), r'F:\桌面\GANPerf\Database_input\label\train_label.csv')
                # real_labels = batch_y
            ...
            mre, mse, rank_spearman, explained_variance_score, mean_absolute_error, root_mean_squared_error, r2_score = model_eval(
                model, valid_loader, use_gpu)
            values.append(mre)
            values.append(mse)
            values.append(rank_spearman)
            ...
            if epoch == 49:
                mre, mse, rank_spearman, explained_variance_score, mean_absolute_error, root_mean_squared_error, r2_score = test_eval(
                    model, test_loader, use_gpu)
            else:
                mre, mse, rank_spearman, explained_variance_score, mean_absolute_error, root_mean_squared_error, r2_score = model_eval(
                    model, test_loader, use_gpu)
            values.append(mre)
            ...
            values.append(explained_variance_score)
            values.append(mean_absolute_error)
            values.append(root_mean_squared_error)
            values.append(r2_score)
            # print(explained_variance_score,mean_absolute_error,root_mean_squared_error,r2_score)
            ...
            temp = pd.DataFrame([values], columns=['epoch', 'train_mse', 'train_rank_spearman',
                                                   'valid_mre', 'valid_mse', 'valid_rank_spearman', 'test_mre',
                                                   'test_mse',
                                                   'test_rank_spearman', 'explained_variance_score',
                                                   'mean_absolute_error',
                                                   'root_mean_squared_error', 'r2_score'])
            record = record.append(temp)

        record.to_csv(r'F:\桌面\GANPerf\results\x264\x='+str(x)+r'\record' + str(k) + '.csv', index=False)
        torch.save(model.state_dict(), 'model_param' + str(k) + '.pkl')
